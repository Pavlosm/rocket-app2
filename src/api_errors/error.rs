...
impl ApiError {
    ...
    pub fn not_found(description: Option<String>) -> ApiError { 
        ApiError::build(404, description)
    }

    pub fn build_with_description(code: u16, description: &str) -> ApiError {
        ApiError::build(code, Some(String::from(description)))
    }

    // building a custom error.
    pub fn build(code: u16, description: Option<String>) -> ApiError {
        let reason: String;
        match code {
